...
//Arayüze menü cartları render edecek fonksiyon

const renderCard = (data) => {

  //Bu fonksiyona dışadıran verilen  datayı dön ve her data için html oluştur

  const cartsHtml = data.map((menu) => `      <a href="/detail.html?id=${menu.id}" class="d-flex flex-column flex-md-row text-dark gap-3" id="card">
        <img
  ...
  elements.menuList.innerHTML = cartsHtml;
}
...
const renderDetailPage = (product, outlet) => {
  ...
  outlet.innerHTML= `
         ...
         <img src="${product.img}" style="max-height: 400px;" 
